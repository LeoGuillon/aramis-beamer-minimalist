...
% commands to make sure the slides number in the appendix aren't counted in the total frames number
...
\begin{frame}
  ...
  Body text font : IBM Plex (there some other fonts in the \texttt{assets/fonts} folder, to customize your presentation).
  \vfill
  \lipsum[1][1-3]
  \vfill
  ...
  \vfill
  \strong{This is some text with strong emphasis.}
  \vfill
  Typographic advice : never use \textbf{\textit{italic bold text}} or \underline{underline text} in presentation ; also, tend to use semantic commands such as \emph{emphasis} instead of \textit{italic} or \textbf{bold}.

\end{frame}

\begin{frame}{Code blocks}
  Monospace font : \texttt{IBM Plex Mono}.
...
\begin{frame}{Appendix frame}
  ...
  If you use the total frame number option, slides in the appendix won't be counted in the total frames number.
\end{frame}
\backupend
